...
LOG_LEVEL CAsyncLog::m_nCurrentLevel = LOG_LEVEL::LOG_LEVEL_INFO;
int64_t CAsyncLog::m_nFileRollSize = DEFAULT_ROLL_SIZE;
int64_t CAsyncLog::m_nCurentWrittenSize = 0;
...
bool CAsyncLog::init(const char* pszLogFileName, bool bTruncateLongLine , int64_t nRollSize)
{
	m_bTruncateLongLog = bTruncateLongLine;
	m_nFileRollSize = nRollSize;

	//初始化文件名称
	if (nullptr == pszLogFileName || pszLogFileName[0] == 0) {
		m_strFileName.clear();
	}
	else
	{
	...
	char szPID[8];
    #ifdef WIN32
	snprintf(szPID, sizeof(szPID), "%05D", static_cast<int>(::GetCurrentProcessId()));
    #else
...
{
	if (nLevel < LOG_LEVEL::LOG_LEVEL_TRACE || nLevel > LOG_LEVEL::LOG_LEVEL_FATAL)
		return;
...
{
   //日志级别判断
	if (nLevel != LOG_LEVEL::LOG_LEVEL_CRITICAL) {
	...
	//容量必须算上最后一个\0
	if (strLogMsg.capacity() < (nLogMsgLength + 1)) {
	...
	if (!m_strFileName.empty())
		strLine += "\n";
	if (nLevel != LOG_LEVEL::LOG_LEVEL_FATAL){
	    ...
	    //为了让FATAL级别的日志能crash程序，采取同步写日志的方法
...
void CAsyncLog::makeLinePrefix(long nLevel, std::string& strPrefix)
{
	//级别
	strPrefix = "[INFO]";
	if (nLevel == LOG_LEVEL::LOG_LEVEL_TRACE)
		strPrefix = "[TRACE]";
	else if (nLevel == LOG_LEVEL::LOG_LEVEL_DEBUG)
		strPrefix = "[DEBUG]";
	else if (nLevel == LOG_LEVEL::LOG_LEVEL_WARNING)
		strPrefix = "[WARN]";
	else if (nLevel == LOG_LEVEL::LOG_LEVEL_ERROR)
		strPrefix = "[ERROR]";
	else if (nLevel == LOG_LEVEL::LOG_LEVEL_SYSERROR)
		strPrefix = "[SYSE]";
	else if (nLevel == LOG_LEVEL::LOG_LEVEL_FATAL)
		strPrefix = "[FATAL]";
	else if (nLevel == LOG_LEVEL::LOG_LEVEL_CRITICAL)
		strPrefix = "[CRITICAL]";
...
void CAsyncLog::writeThreadProc() {
	...
	{   
		...
		if (!m_strFileName.empty())
		{
			if (m_hLogFile == nullptr || m_nCurentWrittenSize >= m_nFileRollSize)
			{
				//重置m_nCurrentWrittenSize大小
				m_nCurentWrittenSize = 0;
		...
		if (!m_strFileName.empty())
		{
			...
			m_nCurentWrittenSize += strLine.length();
		}
